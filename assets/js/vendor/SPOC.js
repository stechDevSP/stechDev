...
(function(window, document, SPOC, undefined) {
     ...
     * @return  string
     */
    SPOC.Utils.Conversion.objToQueryStringSearch = function(obj) {
        var str = '';
            ...
            if (cache && cacheResult) {
                ...
                if (SPOC.Mock && SPOC.Mock.active) {
                    ...
                    if (!SPOC.Utils.Url.isSameDomain(url) && url.toLowerCase().indexOf('_api/web') > -1) {
                        ...
                        var accept = url.indexOf('/_api/search/') > -1 ? "application/json;odata=verbose" : "application/json;odata=nometadata"
    ...
    SPOC.SP.Site.prototype.Search = function(searchTerm) {
        ...
        methods.query = function(settings, cache) {
            return new Promise(function(resolve, reject) {
                ...
                searchUrl += settings ? SPOC.Utils.Conversion.objToQueryStringSearch(settings) : '';

                SPOC.Utils.Request.get(searchUrl, false).then(function(result) {
                    result = SPOC.Utils.SP.formatSearchResponse(result);
                    resolve(result);
                }, function(err) {
                    reject(err);
        ...
        /**
     * Queries a SharePont Result Source via REST API
     * @params  Object query filter paramators in obj format
     * @return  promise
     */
    methods.queryResultSource = function(sourceId, settings, cache) {
        return new Promise(function(resolve, reject) {
            var searchUrl = site.url + '/_api/search/query?querytext=%27' + searchTerm + '%27' + '&sourceid=%27' + sourceId + '%27';
            searchUrl += settings ? SPOC.Utils.Conversion.objToQueryStringSearch(settings) : '';
    ...
    SPOC.SP.User.prototype.Profile = function() {
        ...
        methods.isMemberOfGroup = function(groupName, userId, cache) {
            ...
            listUrl += "/_api/web/sitegroups/getByName('" + groupName + "')/Users?$filter=Id eq " + user;
